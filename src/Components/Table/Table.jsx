...
const Table = ({data, headers}) => {
        ...
        const filterEmployee = (e) => {
            ...
            const filter = data && data.filter((item) => {
                return Object.keys(item).some(key => {
                    const itemValue = typeof item[key] === 'string' ? item[key] : String(item[key]);
                    return itemValue.toLowerCase().includes(value);
                });
            });
            // TODO a refactorisé
            if (value === "") {
                setDataEmployee(sortedData().slice(sliceStart, numberEmployee))
                setShowEmployee(numberEmployee)
            } else {
        ...
        const handlePageChange = (page) => {
            ...
            setDataEmployee(sortedData().slice(numberEmployee * (page - 1), numberEmployee * page))
            // affiche le nombre d'employés par page en fonction du nombre total d'employés et du nombre d'employés affichés
            ...
            <div>
                <div className="container-option">
                    <div className="select-option">
                        ...
                        <select data-testid={"numberShow"} value={numberEmployee}
                    ...
                    <div className="input-search">
                        ...
                        <input id="search-employee" onChange={filterEmployee} type="search" name="search-employee"/>
                    </div>
                </div>
                <table>
                    <thead>
                    <tr className="table-head">
                        {headers.map((header, index) => (
                            <th data-testid={header.data} key={index} onClick={() => requestSort(header.data)}>
                                {header.name}
                    ...
                    <tbody>
                    {dataEmployee && dataEmployee.map((item, index) => (
                        <tr key={index}>
                ...
                <div className="container-pagination">
                    <div className="show-pagination">
                        <p>Showing {showEmployee} of {data && data.length} entries</p>
